...
$mail = new PHPMailer(true);
  ...
  if(isset($_POST['submit'])) {
    ...
    $subject = htmlspecialchars(trim($_POST['phone']));
    $mesg = htmlspecialchars(trim($_POST['date']));
    ...
    $message = "<html><body>
           ...
           <tr><td>Phone .</td><td>{$subject}</td></tr>
           <tr><td>Date</td><td>{$mesg}</td></tr>
     </table></body></html>";
    ...
    try {
            ...
            $mail->SMTPDebug   = 0;
            $mail->Username   = 'srannavi026@gmail.com';                             //SMTP username
            $mail->Password   = 'mrio dxxq wbrc uvze';     //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;                     //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
            $mail->Port       = 587;                                   //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

            //Recipients
           $mail->setFrom($email, $name);
           $mail->addAddress('srannavi026@gmail.com');     //Add a recipient
                      //Name is optional
